...
* id_token name claim namespace
* Resource servers SHOULD cache valid and identities fields for no
longer than 30 seconds
* "email address (e.g., InCommon eduPersonPrincipleName)" but please
note that http://macedir.org/specs/eduperson/ says, "ePPN looks like an
email address but is not intended to be a person's published email
address or be used as an email address." InCommon IdPs provide email
addresses in the "mail" attribute.
* Change "downstream scopes" to "dependent scopes"
* When using a refresh token, we will not invalidate the old one and we
will not issue a new refresh token.
* A refresh token is good for 6 months after the last use
* client credential grant: issues access token, good for a longer
period, can be cached
* behavior when requesting multiple scopes from different resource
server
* Globus Auth Scopes and Grants:
https://gist.github.com/bjmc/5c8aff277ad3b5dbb3d5
* Drop client credentials grant.  Clients and resource servers should
instead just use client id and client secret, as a Basic Authorization
header.  Anywhere there is a client access token, change it to Basic
Auth with client id & secret
* Add the scopes we support

* OIDC scopes
* auth scopes

* Change "user@domain" to "standard"?
* Add/Improve effective identity description in introduction
* Incorporate Globus Auth internal API details, for how transfer is
using Globus Auth

*
https://www.google.com/url?q=https://docs.google.com/document/d/1aa9E_ajjQkl3h__AAMacIx81w8Y3HL3WQp9Y83GQL4w/edit&sa=D&ust=1459362629240000&usg=AFQjCNG0YG7C0bpyP54UapUQuS3jhnQv8w[https://docs.google.com/document/d/1aa9E_ajjQkl3h__AAMacIx81w8Y3HL3WQp9Y83GQL4w/edit]

* Change "identity username" to "identity username" throughout spec
* Token revocation:
https://www.google.com/url?q=https://tools.ietf.org/html/rfc7009&sa=D&ust=1459362629242000&usg=AFQjCNGAMN8yaunLb4ySZkSm3vF9cSX76w[https://tools.ietf.org/html/rfc7009] 
* Dependent token grant: Add description of access_type=offline
* Identity usernames are case insensitive

Later:

* Spec for using OAuth2 with mobile: https://tools.ietf.org/html/rfc7636

link:#cmnt_ref1[[a]]Highlighting so this doesn't slip into anything we
release.

link:#cmnt_ref2[[b]]identity_provider has been removed from current
version

link:#cmnt_ref3[[c]]Changing from "name"

link:#cmnt_ref4[[d]]Changing from "display_name'

link:#cmnt_ref5[[e]]Was identity_names

link:#cmnt_ref6[[f]]was identity_names

link:#cmnt_ref7[[g]]Was "include"

link:#cmnt_ref8[[h]]Was display_name

link:#cmnt_ref9[[i]]This is currently
https://auth.globus.org/grants/dependent_token in beta.  Change it for
first release

link:#cmnt_ref10[[j]]This is currently "assertion=" in beta.  Change it
for first release.

link:#cmnt_ref11[[k]]Stuff like this probably should be moved out of the
spec and into an integration doc.