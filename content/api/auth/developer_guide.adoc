...
2. <<obtaining-authorization,Obtaining authorization>>. Applications request access to _scopes_ using a standard OAuth 2 flow.
3. <<using-tokens,Using tokens>> to authenticate to services.

Those are the basic. Apps that want to provide their users with a more complete, integrated experience can then start looking into features such as <<skinning,skinning>> and <<other-features,other features>> such as allowing users to initiate linking identities from right inside your app.
