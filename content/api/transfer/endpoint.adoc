...
Get a single endpoint by id (or +canonical_name+ for backward compatibility).
All fields are included by default, including the +server+ subdocuments,
but the +fields+ query parameter can be used to fetch only specific fields.
Use 'server' in the fields list to include server subdocuments in a limited
field list.
