...
NOTE: When creating a shared endpoint, you must use the +host_endpoint+ field,
rather than the +host_endpoint_id+ field, but it should still contain the
endpoint id, not a canonical name. This is a legacy wart, and in the next
release +host_endpoint_id+ will work as well.
