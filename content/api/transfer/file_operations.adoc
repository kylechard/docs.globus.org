...
Results can be paged, sorted, and filtered. By default all results are
returned, sorted by (+type+, +name+). Note that this API is not designed to
handle very large directories, and may time out for directories with more than
100,000 entries.
...
| limit  | int
| Change the page size. Defaults to 0 to indicate no limit.

| offset | int
| If using a +limit+ other than 0, this can be used to page through the
  results. Note that the entire directory contents is still fetched from
  the endpoint server on every request. This is because the GridFTP protocol
  used by most endpoints does not support paging, so paging must be
  implemented in the REST API server.

| orderby | string
| [DEPRECATED] A comma separated list of order by options. Each order
...
| filter | string
| [DEPRECATED] Return only file documents that match all of the specified
