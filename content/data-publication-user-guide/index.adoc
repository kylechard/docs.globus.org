...
link:#glossary-pid[persistent identifier] will be assigned which links
...
dataset.  This text will be included in an email to the user informing
them that their dataset submission was rejected.  The rejected
submission will then appear on the user's dashboard where they can
re-open it to update and re-submit or remove it.
...
// Jim commented out for decent formatting right now
// == Dataset Landing Page
