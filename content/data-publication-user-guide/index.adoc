...
take you to the dashboard page.
...
A progress bar at the top of each page shows the steps of the 
submission workflow:
...
and still to be performed steps are shown in grey.
You can return directly to any previously performed step to
review or revise your entries by clickling on the (light blue) 
step name in the progress bar.

The bottom right of many submission pages contains navigation buttons:
...
and `Save & Exit` saves the workflow and exits, 
returning you to the publication dashboard.
`Save & Exit` is particularly helpful if a long-running operation,
such as a large transfer, is taking place and you want to exit
and return later to check progress and continue.
