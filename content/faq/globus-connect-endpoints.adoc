...
This message means that the Globus Connect Personal (GCP) software is not currently running on the workstation where it was installed. In order for any transfers to or from a GCP endpoint, the endpoint needs to be running. This is different from activated.
...
- (link:https://docs.globus.org/how-to/globus-connect-personal-cli/[Running Globus Connect section]) "Both lights green indicate that Globus Connect Personal is running properly and is connected to Globus. The user may minimize or hide this status window once Globus Connect Personal has connected successfully."
