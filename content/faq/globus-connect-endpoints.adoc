...
The Globus webapp provides an interface to manage roles on the endpoint on the link:https://www.globus.org/app/endpoints[Manage Endpoint page]. You can see a description of the supported roles link:../../resource-provider-guide/#roles_and_privileges[here]. Once you select your endpoint, you can choose the Roles tab to grant/revoke various roles on the endpoint.
