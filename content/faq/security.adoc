...
We've moved Globus username/passwords created prior to February 13, 2016 into an independent service called Globus ID (link:https://globusid.org[globusid.org]), which we continue to operate for those users that choose not to use their organization's login to access Globus. For example, if you had previously created the Globus username "auser", you can now log into Globus as auser@globusid.org with your existing Globus password. 
