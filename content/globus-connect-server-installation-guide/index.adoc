...
You will need the link:https://fedoraproject.org/wiki/EPEL[EPEL repository] available and enabled on your system. If this is not already the case, you can install the EPEL repository with the instructions below.

Install EPEL repository on CentOS 5, Red Hat Enterprise Linux 5, and Scientific Linux 5:

----terminal
$ sudo curl -LOs https://dl.fedoraproject.org/pub/epel/epel-release-latest-5.noarch.rpm
$ sudo yum install epel-release-latest-5.noarch.rpm
...
The steps for doing an update are found xref:update_section[here]. We also recommend that you go link:https://www.globus.org/mailing-lists[here] and subscribe to the Admin mailing list to receive Globus release notes.
