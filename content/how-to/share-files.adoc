...
. Highlight the folder that you would like to share, and select the "share" option from the top level. If the "share" option is not enabled, the endpoint is not configured for sharing. Please contact your endpoint administrator or refer to link:../../globus-connect-server-installation-guide#how_to_enable_and_disable_sharing_in_globus_connect_server[Globus Connect Server Installation Guide] for information on how to update the endpoint configuration.
+
[role="img-responsive center-block"]
