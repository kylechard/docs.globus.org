...
This connector enables use of a Globus data access interface on a link:https://www.spectralogic.com/products/blackpearl/[SpectraLogic Black Pearl] storage system. This requires the installation of Globus Connect Server and an additional package that is specific to the SpectraLogic BlackPearl storage system called the SpectraLogic BlackPearl DSI.
