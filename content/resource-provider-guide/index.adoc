...
Please xref:contact_us[contact us] if you have problems understanding
...
Before you can install Globus Connect Server on your server, you must ensure that 
other hosts on the Internet can initiate connections to your server. If your 
server is behind a network address translation (NAT) firewall/router, then xref:nat_section[see below]. 
Otherwise, you must ensure that your server
...
xref:contact_us[Contact us] if you need assistance.
...
[[open-tcp-ports]]
=== Open TCP ports
If your system is behind a firewall, specified TCP ports must be open for Globus to work.
The default Globus Connect Server installation requires these TCP ports be open:
...
* This is to pull cert information from our backend
...
* This is to pull packages from our repository
...
xref:organization-account-anchor[organization's Globus account].
...
You (or any user on your system who has signed up for a Globus account) should now be able to access the Globus endpoint you just created by navigating to the Globus link:https://www.globus.org/xfer/StartTransfer[Transfer Files] page. 
...
To stay current, we encourage you to update your local installation from the Globus repository at least every 3 months. 
...
link:{provider_plans}[*Provider Plan*] subscriptions come with a guaranteed support service level.
...
=== Requirements
Sharing can only be enabled on a link:http://dev.globus.org/resource-provider-guide/#managed-endpoint-anchor[managed endpoint]. Only organizations with a link:https://www.globus.org/providers/provider-plans[Provider Plan] are able to create managed endpoints.
...
Instructions for how an organization with a provider plan can promote an existing endpoint to managed status can be found link:https://support.globus.org/entries/103577526[here].

After ensuring that ABC University has a provider plan with Globus, Adam will next need to make his endpoint managed. To do this he will log in to the Globus CLI server using the "abcu" Globus account (instructions for how to do this can be found link:http://dev.globus.org/cli/[here]) and he will then execute the following command:
...
Instructions for how to create and manage access to a share on a managed endpoint can be found link:https://support.globus.org/entries/23602336[here].
...
[[managed-endpoint-anchor]]Managed Endpoint::
  ...
  endpoint into a managed endpoint see this link:https://support.globus.org/entries/103577526-How-do-I-convert-an-existing-endpoint-into-a-managed-endpoint-[writeup]. 
...
[[organization-account-anchor]]Organization Account::
  ...
  recognized as being associated with your endpoints, as the name of your
