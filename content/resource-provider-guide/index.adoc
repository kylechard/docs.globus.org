...
Please xref:contact-us[contact us] if you have problems understanding
...
xref:contact-us[Contact us] if you need assistance.
...
Globus can be successfully installed at these sites, but, you will need to the edit the 
configuration file (a default config will not work). Please see the xref:nat_section[NAT] section if your site uses NAT.
...
Improvements to the set of software components that make up Globus Connect Server are updated and released frequently. To stay current, we encourage admins to update your local installation with that from the Globus repo at least every 3 months. The steps for doing this are xref:update_section[here].
...
==== +Sharing+ option - link:{provider_plans}[*Provider Plan*] feature
...
==== +SharingRestrictPaths+ option - link:{provider_plans}[*Provider Plan*] feature
This option is used to both enable and/or disable specified file paths for all Globus users that are authorized to use this xref:shared-endpoint-anchor[shared endpoint]. By default, all paths are enabled for access. NOTE: When accessing a Shared Endpoint through Globus, the Sharing Restrict Paths can only set further access restrictions to the Endpoint Owner’s local unix file system permissions.  Globus does not and can not override the local unix file system permissions. This option is only used on a xref:managed-endpoint-anchor[managed endpoint], and will be ignored otherwise.
...
==== Verifying globus-gridftp-server Service Is Running
Another important test to run on servers hosting a Globus endpoint is to verify that the 
globus-gridftp-server service has properly started and is running. One thing to check is to use the `ps` command to see if there is an instance of globus-gridftp-server running:
...
If you do not see an instance of globus-gridftp-server running, then the service has not started. You can try to start it by running the `globus-connect-server-setup` command and then checking to see if an instance of globus-gridftp-server appears in the `ps` output. If you still don't see an instance of globus-gridftp-server running after using the `globus-connect-server-setup` command, you can take a look in the xref:gridftp_server_log_files[logs] for clues as to what might be wrong.

Provided that there is an instance of globus-gridftp-server running, you can then check to see if you can at least connect to it locally via the `telnet` command: 
...
If there is an instance of globus-gridftp-server running, but you can't connect locally, then there is probably a local firewall rule on the server that is interfering with your attempt to connect. If you are successful with this test, then try it again using the public FQDN associated with your server and see if that works as well. If you're not able to connect when using the public FQDN, then you may have a firewall issue or a name resolution issue that is preventing you from being able to connect properly.

==== Verifying myproxy-server Service Is Running
Another important test to run on servers hosting a Globus endpoint is to verify that the 
myproxy-server service has properly started and is running. Not all endpoints will use the MyProxy service, so this test is only useful if your endpoint uses MyProxy for endpoint authentication. One thing to check is to use the `ps` command to see if there is an instance of myproxy-server running:
...
If you do not see an instance of myproxy-server running, then the service has not started. You can try to start it by running the `globus-connect-server-setup` command and then checking to see if an instance of myproxy-server appears in the `ps` output. If you still don't see an instance of myproxy-server running after using the `globus-connect-server-setup` command, you can take a look in the xref:myproxy_server_log_files[logs] for clues as to what might be wrong.

Provided that there is an instance of myproxy-server running, you can then check to see if you can at least connect to it locally via the `telnet` command:
...
If there is an instance of myproxy-server running, but you can't connect locally, then there is probably a local firewall rule on the server that is interfering with your attempt to connect. If you are successful with this test, then try it again using the public FQDN associated with your server and see if that works as well. If you're not able to connect when using the public FQDN, then you may have a firewall issue or a name resolution issue that is preventing you from being able to connect properly.

[[globus-help-resources]]
=== Globus Help Resources

==== Globus Knowledge Base and Forums
The link:https://support.globus.org/[Globus Knowledge Base and Forums] contain a wealth of information about 
Globus services and software. 
Many common issues can be resolved quickly by searching or browsing these resources and reading 
the relevant articles. 
We recommend consulting the Globus Knowledge Base and Forums first when looking for fast resolution to any issue you are having with Globus services or software.
...
used in the globus.org software-as-a-service (SaaS) or platform-as-a-service (PaaS) offering
can be directed to the Globus support team by submitting a link:https://www.globus.org/contact-us[ticket].
{provider_plans}[Provider Plan subscribers] have a guaranteed support service level as part of their subscription.
...
==== Globus Toolkit Mailing List
Questions or issues that pertain to Globus Toolkit components that are not relevant to the globus.org·
software-as-a-service (SaaS) or platform-as-a-service (PaaS) offering should be submitted to the·
gt-user@lists.globus.org email list.·

[[sharing_section]]
== Sharing
Sharing is a link:{provider_plans}[*Provider Plan*] feature, available only on xref:managed-endpoint-anchor[Managed Endpoints], that makes it very simple to share particular folders on an existing endpoint with other Globus users. You can read more about the details and benefits of Globus sharing link:https://globus.org/data-sharing[here]. Instruction on how to create a share on an exiting managed endpoint can be found link:https://support.globus.org/entries/23602336[here].

== Management Console
The Management Console is a link:{provider_plans}[*Provider Plan*] feature, available only on xref:managed-endpoint-anchor[Managed Endpoints], that provides a graphical web interface used to monitor activity on Endpoints, as well as to identify and troubleshoot faults that may indicate underlying infrastructure issues. An organization’s Provider Plan support contact(s) make the decision who has access to the Management Console. You can read more about the details and benefits of the Management Console link:https://globus.org/blog/globus-management-console[here].
...
[[managed-endpoint-anchor]]Managed Endpoint::
  ...
  link:https://www.globus.org/providers/provider-plans[provider plan] that 
  allows advanced features (like xref:sharing_section[Sharing]) to be enabled. To convert an existing 
  endpoint into a managed endpoint see this link:https://support.globus.org/entries/103577526-How-do-I-convert-an-existing-endpoint-into-a-managed-endpoint-[doc]. 
...
[[shared-endpoint-anchor]]Shared Endpoint::
  ...
  then grant read/write permissions, or delegate the xref:access-manager-anchor[access manager] role, 
