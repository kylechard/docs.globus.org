...
{commercial_plans}[commercial subscriptions] help ensure that Globus can
...
link:https://www.globus.org/contact-us[Contact us] if you have problems understanding
...
If you are installing on an link:http://aws.amazon.com/ec2/[Amazon EC2] instance, 
...
default configuration to install Globus; please see the configuration instructions in the xref:nat_section[NAT/firewall] section.
...
link:https://www.globus.org/contact-us[contact us].
...
* We strongly recommend the use of the default port range (read why xref:data_channel_traffic[here])
- Port 2223 outbound to 184.73.255.160
* Used to pull certificate information from the Globus backend.
- Port 443 outbound to nexus.api.globusonline.org and 174.129.226.69
* Used to communicate with the Globus REST API.
...
* Used to pull packages from the Globus repository.
...
the Globus service and software. 
Many common issues can be resolved quickly by searching or browsing these resources and reading 
the relevant articles. 
We recommend consulting the Globus Knowledge Base and Forums first when looking for fast resolution to any issue you are 
having with the Globus service or software.
