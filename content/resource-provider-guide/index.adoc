...
that both ep1 and ep2 are listening to on port 2811. At that point, ep1 would then initiate a 
...
control channel that both ep1 and ep2 are listening to on port 2811. At that point, ep2 would 
...
data port range, as well as be able to make outbound connections on the data port range of any 
...
sites and endpoint may not be willing to set up custom firewall rules to accommodate custom 
