...
Then, edit the Globus Connect Server configuration file installed at /etc/globus-connect-server.conf,
set Name to your choice (geosciences in the example shown), and Public to True.
These two changes will allow authorized users to find and access your endpoint.
----
[Endpoint]
Name = geosciences
Public = True
...
We recommend that you confirm your endpoint is functioning properly by running a couple of test transfers, as described 
...
To stay current, we encourage you to update your local installation from the Globus repo at least every 3 months. 
