...
=== Requirements
Sharing can only be enabled on a link:http://docs.globus.org/resource-provider-guide/#managed-endpoint-anchor[managed endpoint]. Only organizations with a link:{subscription_url}[subscription] are able to create managed endpoints.
...
An organization's link:{subscription_url}[subscription] 
