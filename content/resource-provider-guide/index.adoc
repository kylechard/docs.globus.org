...
One important check to verify endpoint health is to confirm that the endpoint is able to successfully conduct transfers to and from other endpoints.
...
A good test is to attempt to transfer the contents of the `/share/godata/` directory on the go#ep1 endpoint to your own endpoint. 
After that, attempt to transfer those same files to the `/~/` directory on the go#ep2 endpoint. 
If these tests succeed, then you know that your endpoint is functional and is able to serve as the destination and source of transfers. 
