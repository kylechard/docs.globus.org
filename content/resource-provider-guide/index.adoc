...
where [uservars]#'xxx.yyy.zzz'# is
the output of the previous +hostname -f+ command and '8.8.4.4'
specifies one of Google's public DNS servers:
----terminal
$ nslookup [input]#'xxx.yyy.zzz'# 8.8.4.4
----terminal
If you get a message of the form '`** server can't find xxx.yyy.zzz: NXDOMAIN'',
your system is not accessible and you need to 
...
[uservars]#ucrcc# as their master Globus username.
...
then some Globus components are currently installed.
Please refer to the 
section link:#removing_previously_installed_globus_components[Removing 
previously installed Globus components]
and follow the instructions there before continuing.
...
system:
[yellow-background]#should the one-time commands to add repo to pkg mngmnt system be moved up to pre-reqs?  Or is that just harder to follow?#
----terminal
...
<Note that default was MyProxy & PAM.  Maybe include a section for it 
here as well in case they change then want to go back.>

==== Using MyProxy and OAuth
<note that other port needs to be opened beyond what those listed earlier for default install. 
and that they can close the one for PAM if desired.>
x
