...
The Globus team is reaching out now to Globus service admins to check your host certificates and update them if necessary.  We are asking admins to replace any incompatible certificates by March 1, 2016. Then sometime after March 1, we will schedule a Globus Toolkit release that will change the default client authorization algorithm to strict mode.  At that time, the Globus.org transfer service will also update its’ identity checking algorithm. This should allow for no service disruptions for the Globus Community.
